...
GameInitState::GameInitState(SDL_Renderer* renderer, FileManager* fileManager, GameObjects* gameObjects) 
	: GameState(renderer), _fileManager(fileManager), _gameObjects(gameObjects), _platformSpeed()
{
}
...
void GameInitState::DoStart()
{
	...
	controllerPlayer2->AddAxisActionKeys(AxisName::VERTICAL, SDLK_UP, SDLK_DOWN);
	controllerPlayer2->AddActionKey(ActionName::PAUSE, SDLK_p);
	controllerPlayer2->AddActionKey(ActionName::RESUME, SDLK_l);
...
void GameInitState::InitPlayers(Controller* controllerPlayer1, Controller* controllerPlayer2)
{
	...
	_gameObjects->_player1->GetPlatform()->Init(_renderer, Vector2D<int>(PLATFORM_SOURCE_HEIGHT, (SCREEN_HEIGHT / 2) - PLATFORM_DESTINATION_WIDTH),
		PLATFORM_DESTINATION_SIZE, _platformSpeed);

	_gameObjects->_player2 = new Player(controllerPlayer2);
	_gameObjects->_player2->GetPlatform()->Init(_renderer, Vector2D<int>(SCREEN_WIDTH - 80, (SCREEN_HEIGHT / 2) - PLATFORM_DESTINATION_WIDTH),
		PLATFORM_DESTINATION_SIZE, _platformSpeed);
}

void GameInitState::InitBall()
{
	_gameObjects->_ball = new Ball(Vector2D<int>(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2), _platformSpeed);
